...
$conn = mysqli_connect("localhost", "root", "admin", "rating") or die("Can connect to database");
function getRatingInfo($id)
{
    global $conn;
    $query = "select * from rating.rating_info where product_id=" . $id;
...
{
    global $conn;
    $query = "select rate_" . $rate . "from rating.rating_info where product_id = " . $id;
    $result = mysqli_query($conn, $query);
    $row = mysqli_fetch_array($result);
    $i = $row['rate_' . $rate];
    $i++;
...
{
    global $conn;
    $query = "SELECT * FROM rating.products WHERE id=" . $id;
    $result = mysqli_query($conn, $query);
    $row = mysqli_fetch_array($result);
...
{
    global $conn;
    $query = "select * from rating.products";
    $result = mysqli_query($conn, $query);
